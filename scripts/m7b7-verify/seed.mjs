// M7 批次7 实机核验种子：建隔离测试书 m7b7 + 三短章（自定义字段/取名/自由摆位/参考浮窗用）。
// 用完 cleanup.mjs 清干净（不动用户原有书）。用法：node scripts/m7b7-verify/seed.mjs
...
if (stale) {
  db.prepare("DELETE FROM books WHERE id = ?").run(stale.id); // 级联清 chapters/custom_field_defs
}
const bookId = db
...
const chapters = [
  { key: "jia", title: "短章甲", sort: 1, paras: ["山间小径泥泞，独行者裹紧斗篷。", "溪水声近了。"] },
  { key: "yi", title: "短章乙", sort: 2, paras: ["渡口的灯笼次第亮起。", "船家说，今夜不走。"] },
  { key: "bing", title: "短章丙", sort: 3, paras: ["城头的旗换了颜色。"] },
];
